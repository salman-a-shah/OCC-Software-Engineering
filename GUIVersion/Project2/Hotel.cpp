...
reservationIter Hotel::findReservation(string name)
{
	for (int i = 0; i < CAPACITY; i++)	
	{
		...
		{
			if((*iter)->getGeneralInfo().getName() == name)
			{
				break;
			}
			else
...
// This function will delete a roomType object and return true if successful
bool Hotel::deleteAccount(const string& newName)
{
	accountsIter iterAccounts = findAccount(newName);
...
bool Hotel::modifyAccount(Accounts& modifiedAccount)
{
	accountsIter iterAccounts = findAccount(modifiedAccount.getName());

	if(iterAccounts != allAccounts.end())
	{
		(*iterAccounts)->setUsername(modifiedAccount.getUsername());
		(*iterAccounts)->setPassword(modifiedAccount.getPassword());
		(*iterAccounts)->setAccountType(modifiedAccount.getAccountType());
		(*iterAccounts)->setName(modifiedAccount.getName());
		(*iterAccounts)->setAge(modifiedAccount.getAge());
		(*iterAccounts)->setGender(modifiedAccount.getGender());
		(*iterAccounts)->setPosition(modifiedAccount.getPosition());
		(*iterAccounts)->setPhoneNumber(modifiedAccount.getPhoneNumber());
		(*iterAccounts)->setEmergencyContact(modifiedAccount.getEmergencyContact());
		(*iterAccounts)->setEmailAddress(modifiedAccount.getEmailAddress());
		(*iterAccounts)->setAddress(modifiedAccount.getAddress());
		(*iterAccounts)->setCity(modifiedAccount.getCity());
		(*iterAccounts)->setZipcode(modifiedAccount.getZipcode());
		(*iterAccounts)->setState(modifiedAccount.getState());
		(*iterAccounts)->setCountry(modifiedAccount.getCountry());
		(*iterAccounts)->setAdditionalDetails(modifiedAccount.getAdditionalDetails());
		db->updateAccount(db, *iterAccounts, &modifiedAccount);
		return true;
	}
	return false;
}

accountsIter Hotel::findAccount(const string& newName)
{
...
accountsIter Hotel::findAccountByUsername(const string& username)
{
	accountsIter iter = allAccounts.begin();
...
accountsIter Hotel::getAccountsBegin()
{
	return allAccounts.begin();
}

accountsIter Hotel::getAccountsEnd()
{
	return allAccounts.end();
}

bool Hotel::accountExists(const string& newName)
{
	if (findAccount(newName) != allAccounts.end())
	{
		return true;
	}
	return false;
...
bool Hotel::accountExistsByUsername(const string& username)
{
	if (findAccountByUsername(username) != allAccounts.end())
	{
		return true;
	}
	return false;
...
vector<RoomType*> Hotel::getAllRoomTypes() const
{
	return allRoomTypes;
}

vector<Room*> Hotel::getAllRoom() const
{
	return allRooms;
}

list<Reservation*> Hotel::getCalendarArray()
{
